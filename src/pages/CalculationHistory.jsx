...
const KEY = 'calculationhistory-records'

export default function CalculationHistoryPage() {
  const [items, setItems] = useState([])
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({'id': '', 'user_id': '', 'session_id': '', 'request_id': '', 'result_id': ''})
  ...
  function save() {
    ...
    setShowForm(false); setEditing(null); setForm({'id': '', 'user_id': '', 'session_id': '', 'request_id': '', 'result_id': ''})
  }
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">CalculationHistory</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage CalculationHistory records</p>
        </div>
        <button onClick={() => { setShowForm(true); setEditing(null); setForm({'id': '', 'user_id': '', 'session_id': '', 'request_id': '', 'result_id': ''}) }} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[85vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-gray-900 mb-4">{editing ? 'Edit' : 'New'} CalculationHistory</h2>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">result_id</label>
              <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form['result_id'] || ''} onChange={e => setForm(p => ({...p, 'result_id': e.target.value}))} />
            </div>
            </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl">
          <p className="text-gray-400 text-sm">No CalculationHistory records yet.</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-x-auto">
          <table className="w-full min-w-[480px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">result_id</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{String(item['result_id'] ?? '')}</td>
